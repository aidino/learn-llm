...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.gemma3n.modeling_gemma3n import (F, Callable, Sequence, Optional, Union, torch, nn, ACT2FN, Cache, HybridCache, GenerationMixin, FlashAttentionKwargs, BaseModelOutputWithPast, ModelOutput, CausalLMOutputWithPast, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, can_return_tuple, AutoModel, Gemma3nAudioConfig, Gemma3nConfig, Gemma3nTextConfig, Gemma3nVisionConfig, logger, __name__, Gemma3nModel, Gemma3nCausalLMOutputWithPast, Gemma3nPreTrainedModel, Gemma3nTextModel, Gemma3nForCausalLM, Gemma3nForConditionalGeneration, Gemma3nAudioConformerLightConv1d, Gemma3nAudioRelativePositionEmbedding, Gemma3nAudioSSCPConvBlock)
    ...
    **loss_kwargs,
    ...
    if RETURN_HIDDEN_STATES:
        ...
        if (self.config.final_logit_softcapping) not in (None, (),):
            ...
            logits = torch.tanh(logits)
            logits = logits * (self.config.final_logit_softcapping)
        loss = self.loss_function(logits, labels.to(self.lm_head.weight.device), vocab_size=self.vocab_size, **loss_kwargs)
...
class Gemma3nForCausalLM(Gemma3nPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        return Gemma3nForCausalLM_forward(self, input_ids, attention_mask, position_ids, past_key_values, inputs_embeds, labels, use_cache, output_attentions, output_hidden_states, cache_position, logits_to_keep, **loss_kwargs)
